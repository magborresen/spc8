...
def test_values(radar, states):
    ...
    tau_0 = radar.time_delay(states[0], np.array([0]))
    ...
    last_time = np.array([0, radar.t_obs + radar.k_space])
    #last_time = np.array([np.linspace(0, radar.t_obs + radar.k_space, 10)])
    tau_1 = radar.time_delay(states[0], last_time)
    dist_1 = tau_1 * 300e6 / 2
    
    print('Truth:', true_0, true_1)
    print(last_time)
    print(dist_0,dist_1)
    print(states)
    
    assert dist_0 == true_0
    assert dist_1[1] == true_1
